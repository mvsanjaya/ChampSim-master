...
int main(int argc, char** argv)
{
    ...
    while (run_simulation) {
        ...
        for (int i=0; i<NUM_CPUS; i++) {
            ...
            if (show_heartbeat && (ooo_cpu[i].num_retired >= ooo_cpu[i].next_print_instruction)) {
                ...
                //outfile << "Heartbeat CPU " << i << " instructions: " << ooo_cpu[i].num_retired << " cycles: " << current_core_cycle[i];
                //outfile << " heartbeat IPC: " << heartbeat_ipc << " cumulative IPC: " << cumulative_ipc; 
                //outfile  << " cumulative IPC: " << cumulative_ipc; 
                //outfile << " (Simulation time: " << elapsed_hour << " hr " << elapsed_minute << " min " << elapsed_second << " sec) " << endl;
                //outfile << "\n";
                //outfile << "\n";                 
                //outfile << "\n";                 
                //outfile << "\n";                 
    ...
    if (NUM_CPUS > 1) {
        ...
        for (uint32_t i=0; i<NUM_CPUS; i++) {
    		...
    		std::ofstream outfile;
            outfile.open("ipc_data.txt", std::ios_base::app);
               // outfile << "Heartbeat CPU " << i << " instructions: " << ooo_cpu[i].num_retired << " cycles: " << current_core_cycle[i];
                //outfile << " heartbeat IPC: " << heartbeat_ipc << " cumulative IPC: " << cumulative_ipc; 
                //outfile  << " cumulative IPC: " << cumulative_ipc; 
                //outfile << " (Simulation time: " << elapsed_hour << " hr " << elapsed_minute << " min " << elapsed_second << " sec) " << endl;
            outfile << (float) (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) / (current_core_cycle[i] - ooo_cpu[i].begin_sim_cycle); 
            //outfile << " instructions: " << ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr << " cycles: " << current_core_cycle[i] - ooo_cpu[i].begin_sim_cycle << endl;               
            outfile << "\n";                 
            outfile.close();

			std::ofstream outfile1;
            outfile1.open("All_ipc_data.txt", std::ios_base::app);
               // outfile << "Heartbeat CPU " << i << " instructions: " << ooo_cpu[i].num_retired << " cycles: " << current_core_cycle[i];
                //outfile << " heartbeat IPC: " << heartbeat_ipc << " cumulative IPC: " << cumulative_ipc; 
                //outfile  << " cumulative IPC: " << cumulative_ipc; 
                //outfile << " (Simulation time: " << elapsed_hour << " hr " << elapsed_minute << " min " << elapsed_second << " sec) " << endl;
            outfile1 << (float) (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) / (current_core_cycle[i] - ooo_cpu[i].begin_sim_cycle); 
            //outfile << " instructions: " << ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr << " cycles: " << current_core_cycle[i] - ooo_cpu[i].begin_sim_cycle << endl;               
            outfile1<< "\n";                 
            outfile1.close();
